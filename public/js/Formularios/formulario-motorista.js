    ...
    try {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#uf_nascimento').val(response.data.uf_nascimento);
                $('#municipio_nascimento').val(response.data.municipio_nascimento);
                $('#uf_cnh').val(response.data.uf_cnh);
                $('#municipio_cnh').val(response.data.municipio_cnh);
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (!data.hasOwnProperty('erro')) {
                        ...
                        $('.selectJs').trigger('change');
                        buscarCidadeResidencia(data.uf, data.localidade);
                    }
                },
                error: function () {
        ...
        $.ajax({
            ...
            success: function (response1) {
                if (response1.data) {
                    ...
                    $('.selectJs').trigger('change');
                    buscarCidadeResidencia(response1.data.uf, response1.data.cidade);
...
function buscarCidadeResidencia(uf, cidade) {
    try {
        $.ajax({
            url: '/consultar-municipios/' + uf,
            dataType: 'json',
...
$(document).ready(function () {
    ...
    $('#uf').on('select2:close', function (e) {
        buscarCidadeResidencia($('#uf').val());
    });

    $('#uf_nascimento').on('select2:close', function (e) {
        buscarMunicipioNascimento($('#uf_nascimento').val());
    });

    $('#uf_cnh').on('select2:close', function (e) {
        buscarMunicipioCNH($('#uf_cnh').val());
    });

    $('#status').on('blur', function () {
        exibirCamposInativos($('#status').val());
    });
